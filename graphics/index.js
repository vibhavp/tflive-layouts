(function() {
	...
	let mapsDiv = document.getElementById("maps");
	...
	function setProfiles(id, role) {
		nodecg.Replicant(id+'_info', 'tflive').on('change', data => {
			if (data === null || data === undefined || data === {}) {
				return;
			}
			console.log(data);
			$($('#'+id).children()[0]).attr('src', data.twitter_img);
			$('#'+id).children()[1].innerText = data.name+'<br>@'+data.twitter+'<br>'+role;
		})
	}

	setProfiles('caster1');
	setProfiles('caster2');
	setProfiles('observer');
})();
